...
function renderDetalleHorasGrid(
  filas: HorasExtraEmpleadoFilaForm[],
  diasColumnas: DiaColumnaHoras[],
): string {
...
function renderDetalleModalBody(state: HorasExtraSolicitudPageState): string {
  if (state.detalleStatus === "loading") {
    return `<p class="text-sm text-text-secondary">Cargando detalle…</p>`;
  }
  if (state.detalleStatus === "error") {
    return `<p class="text-sm text-red-700">${escapeHtml(state.detalleError ?? "No se pudo cargar el detalle.")}</p>`;
  }
...
function renderDetalleModal(state: HorasExtraSolicitudPageState): string {
  ...
  const det = state.detalleAbierto;
  const showFooter = state.detalleStatus === "idle" && Boolean(det);

  return `
    <div id="he-sup-detalle-backdrop" class="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/40 p-4" role="presentation">
      <div
        class="flex max-h-[90vh] w-full max-w-3xl flex-col overflow-hidden rounded-xl border border-slate-200 bg-white shadow-xl"
        role="dialog"
        aria-modal="true"
        aria-labelledby="he-sup-detalle-title"
      >
        <header class="flex shrink-0 items-center justify-between gap-3 border-b border-slate-100 px-5 py-4">
          <h2 id="he-sup-detalle-title" class="text-lg font-semibold text-[#0A1628]">
            ${det ? `Solicitud #${det.id}` : "Detalle de solicitud"}
          </h2>
          <button
            type="button"
            data-he-detalle-cerrar
        ...
        <footer class="flex shrink-0 justify-end border-t border-slate-100 px-5 py-4">
          <button type="button" data-he-detalle-cerrar class="${BTN_SECONDARY} min-h-10 px-5">Cerrar</button>
        </footer>`
            : ""
        }
      </div>
