...
export type LevelUpCategory = {
  id: "talento" | "formacion" | "cumplimiento";
  title: string;
  items: readonly LevelUpAccessItem[];
};

const LEVEL_UP_TALENTO: readonly LevelUpAccessItem[] = [
  {
    id: "puestos",
    key: "puestos",
    href: "#/puestos",
...
const LEVEL_UP_CUMPLIMIENTO: readonly LevelUpAccessItem[] = [
  {
    id: "evidencias",
    key: "evidencias",
    href: "#/evidencias",
...
export const LEVEL_UP_CATEGORIES: readonly LevelUpCategory[] = [
  { id: "talento", title: "Talento", items: LEVEL_UP_TALENTO },
...
export function getVisibleLevelUpCategories(rol: string | null): LevelUpCategory[] {
  return LEVEL_UP_CATEGORIES.map((category) => {
    ...
    if (category.id === "talento" && hasLevelUpResumenModuleAccess(rol)) {
