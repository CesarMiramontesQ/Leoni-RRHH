...
function renderPageSkeleton(): string {
  return `
  <div class="${RH_LISTADO_PAGE_OUTER_GRADIENT}" aria-busy="true" aria-label="Cargando matriz">
...
export function mountCapacidades(container: HTMLElement, signal: AbortSignal): void {
  ...
  function paint(): void {
    ...
    <div class="${RH_LISTADO_PAGE_OUTER_GRADIENT}">
      ${renderLevelUpBackBar()}
      ${talentoEyebrow()}
      ${pageHeading("Matriz de Multihabilidades", "Niveles de competencia por colaborador frente a los requisitos del puesto.")}
