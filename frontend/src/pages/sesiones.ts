...
export function mountSesiones(container: HTMLElement): void {
  ...
  function renderPage(): string {
    ...
    <div class="space-y-6">
      ...
      <div class="flex flex-wrap items-center gap-3">
        <input type="text" data-action="sesiones-search" placeholder="Buscar curso, ubicación, instructor..."
          value="${escapeHtml(state.busqueda)}"
          class="rounded-lg border border-border bg-white px-3 py-2 text-sm w-72 ${FIELD_FOCUS}" />
        <select data-action="sesiones-filter-estado" class="rounded-lg border border-border bg-white px-3 py-2 text-sm ${FIELD_FOCUS}">
      ...
      ${state.loading ? `<p class="text-sm text-slate-400">Cargando...</p>` : state.data.items.length === 0 ? `
      <div class="rounded-2xl border border-border bg-white p-8 text-center">
        <p class="text-sm text-slate-400">Sin sesiones encontradas.</p>
      </div>` : `
      ...
      ${totalPages > 1 ? `
      <div class="flex items-center justify-between">
        <span class="text-xs text-slate-500">Página ${state.page} de ${totalPages}</span>
  ...
  function renderRows(): string {
    return state.data.items.map(s => {
      ...
      return `
      <tr class="border-b border-slate-100 hover:bg-slate-50/60 cursor-pointer" data-action="go-sesion" data-curso-id="${s.curso_id}" data-sesion-id="${s.id}">
        <td class="px-4 py-3">
          <span class="text-sm font-medium text-blue-600">${escapeHtml(s.curso_nombre ?? `Curso #${s.curso_id}`)}</span>
        </td>
        <td class="px-4 py-3 text-sm text-text-primary font-medium">${escapeHtml(fecha)}</td>
        <td class="px-4 py-3 text-sm text-slate-600">${escapeHtml(horario)}</td>
        ...
        <td class="px-4 py-3 text-sm tabular-nums text-slate-600">${cupo}</td>
        <td class="px-4 py-3">
