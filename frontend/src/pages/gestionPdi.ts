...
export function mountGestionPdi(container: HTMLElement, signal: AbortSignal): void {
  ...
  function render() {
    const { resumen, data, areas, filters } = state;
        ...
        <div class="overflow-x-auto rounded-lg border border-slate-200">
          ...
          <table class="min-w-[920px] w-full text-left">
            ...
            <tbody class="divide-y divide-slate-100">
              ...
              <tr class="cursor-pointer hover:bg-blue-50/40" data-action="go-empleado" data-id="${item.empleado_id}">
                ...
                <td class="px-3 py-2.5 align-middle">${badgeHtml(item)}</td>
              </tr>`).join("")}
            </tbody>
          </table>`}
        </div>
