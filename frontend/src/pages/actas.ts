...
import { downloadActasExcel } from "../actas/exportActasExcel.ts";
...
export function mountActas(container: HTMLElement): void {
  ...
  function exportarActasListado(): void {
    const filtered = filterActasRows(allRows, state);
    downloadActasExcel({ rows: filtered });
  }
  ...
  function paint(): void {
    ...
    if (inner) {
      inner.innerHTML = renderActasMain(state, table, allRows, isLoading);
    }
    if (restoreSearch) {
  ...
  pageRoot?.addEventListener("click", async (event) => {
    ...
    if (target.closest("#rh-actas-export")) {
      exportarActasListado();
      return;
    }
