...
export function renderNotificacionListItem(
  ...
  const message = escapeHtml(summarizeNotificacionMessage(item.message, options.compact ? 84 : 180));
  ...
  return `<button type="button" class="${rowBase} ${rowState}" data-notif-id="${item.id}">
