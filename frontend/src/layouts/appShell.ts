...
const NAV_PRIMARY: readonly NavItemDef[] = [
  ...
  },
  {
    id: "comedor",
    key: "comedor",
    hrefFor: (rol) => (rol === "empleado" ? "#/comedor" : "#"),
    label: "Comedor",
...
function footerAjustesHtml(rol: string | null): string {
  if (!isShellNavItemVisibleForRol(rol, NAV_AJUSTES.id)) return "";
  const href = NAV_AJUSTES.hrefFor(rol);
  return `<li class="mt-auto">
    <a href="${href}" class="group -mx-2 flex gap-x-3 rounded-md p-2 text-sm/6 font-semibold text-text-primary hover:bg-surface hover:text-leoni-blue">
      <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" aria-hidden="true" class="${navIconInactive}">
        ${NAV_AJUSTES.svgPaths}
      </svg>
      ${NAV_AJUSTES.label}
    </a>
  </li>`;
