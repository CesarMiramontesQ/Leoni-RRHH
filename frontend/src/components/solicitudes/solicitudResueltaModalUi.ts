...
export function escapeHtmlSr(text: string): string {
  return text
    .replaceAll("&", "&amp;")
    .replaceAll("<", "&lt;")
    .replaceAll(">", "&gt;")
...
function renderTimelineItem(
  ...
  const comentario =
    item.comentario?.trim() ?
      `<blockquote class="mt-2 rounded-lg border border-slate-100 bg-slate-50 px-3 py-2 text-sm italic text-slate-600">"${escapeHtmlSr(item.comentario.trim())}"</blockquote>`
    : "";
  const rail =
    ...
    <li class="flex gap-4">
      ...
      <div class="min-w-0 flex-1 pb-2">
        <div class="flex flex-col gap-1 sm:flex-row sm:items-start sm:justify-between">
          <p class="text-sm font-bold text-slate-900">${escapeHtmlSr(item.titulo)}</p>
          <time class="shrink-0 text-xs text-slate-500">${escapeHtmlSr(item.fecha_hora)}</time>
        </div>
        <p class="mt-1 text-xs text-slate-600">
          <span class="font-medium text-slate-700">${escapeHtmlSr(item.actor_nombre)}</span>
          <span class="text-slate-400"> · </span>
          <span>${escapeHtmlSr(item.actor_rol)}</span>
        </p>
        ${comentario}
      </div>
    </li>`;
      ...
      >
        <header class="flex shrink-0 justify-end border-b border-slate-100 px-5 pt-4 sm:px-8 sm:pt-5">
          <button
            ...
            data-rh-sr-close
            aria-label="${escapeHtmlSr(SR_COPY.cerrarAria)}"
          >
  ...
  return `
    ...
    <div class="mt-6 flex justify-center">
      <button type="button" data-rh-sr-close class="rounded-xl border border-slate-200 bg-white px-5 py-2.5 text-sm font-semibold text-slate-700 shadow-sm hover:bg-slate-50">
        ${escapeHtmlSr(SR_COPY.btnCerrar)}
      </button>
    </div>`;
}
...
export function solicitudResueltaEmptyBodyHtml(): string {
  return `
    <p class="py-16 text-center text-sm text-slate-500">${escapeHtmlSr(SR_COPY.vacio)}</p>
...
function headerInnerHtml(vm: SolicitudResueltaDetalleVm): string {
  ...
  return `
    <div class="flex flex-wrap items-start justify-between gap-3 pr-2">
      <div class="min-w-0">
        <h2 id="rh-sr-title" class="text-xl font-bold tracking-tight text-leoni-blue sm:text-2xl">${escapeHtmlSr(vm.titulo)}</h2>
        <p class="mt-1 text-sm text-slate-500">${escapeHtmlSr(vm.id_etiqueta)}</p>
      </div>
      <span class="${badgeCls} shrink-0">${escapeHtmlSr(badgeTxt)}</span>
    </div>`;
}
  ...
  const rechazoBloque =
    vm.estado_ui === "rechazada" ?
      `<div class="rounded-xl border border-red-200/90 bg-red-50/80 p-4 sm:p-5">
        <h4 class="text-sm font-bold text-red-900">${escapeHtmlSr(SR_COPY.bloqueRechazoTitulo)}</h4>
        <p class="mt-2 text-sm leading-relaxed text-red-900/90">${escapeHtmlSr(vm.motivo_rechazo ?? "—")}</p>
        <dl class="mt-3 grid gap-2 text-xs sm:grid-cols-2">
          <div><dt class="text-slate-600">${escapeHtmlSr(SR_COPY.lblResponsable)}</dt><dd class="font-semibold text-slate-900">${escapeHtmlSr(vm.rechazado_por ?? "—")}</dd></div>
          <div><dt class="text-slate-600">${escapeHtmlSr(SR_COPY.lblFechaRechazo)}</dt><dd class="font-semibold text-slate-900">${escapeHtmlSr(vm.fecha_rechazo ?? "—")}</dd></div>
        </dl>
        ${vm.comentario_rechazo_largo && vm.comentario_rechazo_largo !== vm.motivo_rechazo ? `<div id="rh-sr-rechazo-largo" class="mt-3 hidden rounded-lg border border-red-100 bg-white px-3 py-2 text-sm text-slate-700">${escapeHtmlSr(vm.comentario_rechazo_largo)}</div>` : ""}
      </div>`
    : "";
  ...
  const footerAprobadaProceso =
    vm.estado_ui === "aprobada" && vm.proceso_completado ?
      `<div class="flex flex-col gap-4 border-t border-slate-100 pt-5 sm:flex-row sm:items-center sm:justify-between">
        <p class="text-sm text-slate-600">${escapeHtmlSr(SR_COPY.procesoCompletado)}</p>
        <div class="flex flex-wrap justify-end gap-3">
          ${vm.comprobante_disponible ? `<button type="button" data-rh-sr-descargar class="rounded-xl border border-slate-200 bg-white px-4 py-2.5 text-sm font-semibold text-slate-700 shadow-sm hover:bg-slate-50">${escapeHtmlSr(SR_COPY.btnDescargar)}</button>` : ""}
          <button type="button" data-rh-sr-close class="rounded-xl border border-slate-200 bg-white px-4 py-2.5 text-sm font-semibold text-slate-700 shadow-sm hover:bg-slate-50">${escapeHtmlSr(SR_COPY.btnCerrar)}</button>
        </div>
      </div>`
    : "";
    ...
    vm.estado_ui === "aprobada" && !vm.proceso_completado ?
      `<div class="flex flex-col gap-4 border-t border-slate-100 pt-5 lg:flex-row lg:items-center lg:justify-between">
        <p class="text-sm text-leoni-blue">
          ...
          <span class="font-semibold">${escapeHtmlSr(SR_COPY.siguientePaso)}</span>
          ${vm.siguiente_paso ? ` ${escapeHtmlSr(vm.siguiente_paso)}` : ""}
        </p>
        <div class="flex flex-col-reverse gap-3 sm:flex-row sm:justify-end">
          ${vm.puede_cancelar ? `<button type="button" data-rh-sr-cancelar class="min-h-11 rounded-xl border border-slate-200 bg-white px-5 py-2.5 text-sm font-semibold text-slate-700 shadow-sm hover:bg-slate-50">${escapeHtmlSr(SR_COPY.btnCancelar)}</button>` : ""}
          ${vm.puede_firmar ? `<button type="button" data-rh-sr-firmar class="min-h-11 rounded-xl bg-leoni-blue px-6 py-2.5 text-sm font-bold text-white shadow-md shadow-leoni-blue/20 hover:bg-leoni-blue-light">${escapeHtmlSr(SR_COPY.btnFirmar)}</button>` : ""}
        </div>
      </div>`
    : "";
    ...
    vm.estado_ui === "rechazada" ?
      `<div class="flex flex-col gap-3 border-t border-slate-100 pt-5 sm:flex-row sm:flex-wrap sm:items-center sm:justify-between">
        <div class="flex flex-wrap gap-2">
          ${vm.comentario_rechazo_largo && vm.comentario_rechazo_largo !== vm.motivo_rechazo ? `<button type="button" id="rh-sr-toggle-comentario" class="rounded-xl border border-slate-200 bg-white px-4 py-2 text-sm font-semibold text-slate-700 hover:bg-slate-50">${escapeHtmlSr(SR_COPY.btnVerComentario)}</button>` : ""}
          ${vm.comprobante_disponible ? `<button type="button" data-rh-sr-descargar class="rounded-xl border border-slate-200 bg-white px-4 py-2 text-sm font-semibold text-slate-700 hover:bg-slate-50">${escapeHtmlSr(SR_COPY.btnDescargar)}</button>` : ""}
        </div>
        <button type="button" data-rh-sr-close class="min-h-11 rounded-xl bg-slate-100 px-6 py-2.5 text-sm font-semibold text-slate-800 hover:bg-slate-200">${escapeHtmlSr(SR_COPY.btnCerrar)}</button>
      </div>`
    : "";
  ...
  return `
    ...
    <div class="border-b border-slate-100 pb-5">${headerInnerHtml(vm)}</div>
      <div class="grid grid-cols-1 gap-4 lg:grid-cols-2">
        <div class="${CARD}">
          <h3 class="${CARD_TITLE}">${escapeHtmlSr(SR_COPY.cardGeneral)}</h3>
          <dl class="mt-4 space-y-3">
            <div><dt class="${LBL}">${escapeHtmlSr(SR_COPY.lblEmpleado)}</dt><dd class="${VAL}">${escapeHtmlSr(vm.empleado_nombre)}</dd></div>
            <div><dt class="${LBL}">${escapeHtmlSr(SR_COPY.lblTipoAusencia)}</dt><dd class="${VAL}">${escapeHtmlSr(vm.tipo_ausencia)}</dd></div>
            <div><dt class="${LBL}">${escapeHtmlSr(SR_COPY.lblDepartamento)}</dt><dd class="${VAL}">${escapeHtmlSr(vm.departamento)}</dd></div>
          </dl>
        </div>
        <div class="${CARD}">
          <h3 class="${CARD_TITLE}">${escapeHtmlSr(SR_COPY.cardPeriodo)}</h3>
          <dl class="mt-4 space-y-3">
            <div><dt class="${LBL}">${escapeHtmlSr(SR_COPY.lblDesde)}</dt><dd class="${VAL}">${escapeHtmlSr(vm.fecha_inicio)}</dd></div>
            <div><dt class="${LBL}">${escapeHtmlSr(SR_COPY.lblHasta)}</dt><dd class="${VAL}">${escapeHtmlSr(vm.fecha_fin)}</dd></div>
            <div><dt class="${LBL}">${escapeHtmlSr(SR_COPY.lblTotalDias)}</dt><dd class="text-lg font-bold tabular-nums text-leoni-blue">${escapeHtmlSr(diasTxt)}</dd></div>
          </dl>
        </div>
      </div>
      ...
      <section class="border-t border-slate-100 pt-6" aria-labelledby="rh-sr-hist-title">
        <div class="mb-4 flex flex-wrap items-center justify-between gap-2">
          <h3 id="rh-sr-hist-title" class="flex items-center gap-2 text-base font-bold text-slate-900">
            ...
            ${escapeHtmlSr(SR_COPY.seccionHistorial)}
          </h3>
          <p class="text-xs text-slate-500">${escapeHtmlSr(SR_COPY.actualizado)}${vm.actualizado_relativo ? ` · ${escapeHtmlSr(vm.actualizado_relativo)}` : ""}</p>
        </div>
        <ul class="space-y-0">
          ${timelineItems}
        </ul>
      ...
      ${footerRechazada}
      <input type="hidden" id="rh-sr-solicitud-id" value="${escapeHtmlSr(vm.id)}" />
    </div>`;
}
