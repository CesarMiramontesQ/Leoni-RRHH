...
export const RH_DASH_INC_TENDENCIA_CHART_ID = "rh-dash-inc-tendencia-tipo";
...
export function renderIncidenciasTendenciaPorTipoChart(
  ...
  chartId: string = RH_DASH_INC_TENDENCIA_CHART_ID,
): string {
  const has = data.series.some((s) => s.valores.some((v) => v > 0));
  if (!has || data.periodos.length === 0) {
    return `<div class="flex min-h-[220px] items-center justify-center rounded-lg border border-dashed border-[color:var(--color-border)] bg-white px-4 py-8 text-center text-sm text-[color:var(--color-text-muted)]">Sin incidencias en el periodo</div>`;
  }
  return `
    <div class="rh-inc-chart-panel flex min-h-[220px] w-full min-w-0 flex-1 flex-col justify-center">
      ${renderChartCanvas({
        chartId,
        ariaLabel: "Tendencia de incidencias por tipo",
        heightClass: RH_INC_ANALYTICS_CHART_HEIGHT_CLASS,
        className: "relative w-full min-w-0",
      })}
    </div>`;
