...
function valueFor(id: string, m: EmpleadoIncidenciasMetricas): number {
  if (id === "retardos") return m.retardos;
  if (id === "faltas") return m.faltasJustificadas;
  return m.total;
}

function renderMetricCard(def: MetricCardDef, value: number): string {
  return `
    <article class="rh-dash-kpi-card flex h-full flex-col rounded-[18px] p-5" data-v360-inc-metric="${def.id}">
...
export function vista360IncidenciasMetricasSkeletonHtml(): string {
  ...
  return `
    <div class="grid grid-cols-1 gap-4 sm:grid-cols-3" aria-busy="true" aria-label="Cargando métricas de incidencias">
      ${cell.repeat(3)}
    </div>`;
}

export function vista360IncidenciasMetricasCardsHtml(metricas: EmpleadoIncidenciasMetricas): string {
  const cards = CARDS.map((def) => renderMetricCard(def, valueFor(def.id, metricas))).join("");
  return `
    <div class="grid grid-cols-1 gap-4 sm:grid-cols-3" aria-label="Métricas de incidencias del empleado">
      ${cards}
    </div>`;
}
