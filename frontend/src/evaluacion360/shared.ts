    ...
    ${kpis
      ...
      <div class="rounded-xl border border-border bg-white p-4">
        ...
        <div class="mt-2 flex items-end justify-between gap-2">
          <p class="text-2xl font-bold tabular-nums text-text-primary">${escapeHtml(k.value)}${k.suffix ? `<span class="text-sm font-medium text-slate-400">${escapeHtml(k.suffix)}</span>` : ""}</p>
          ${eval360Sparkline(k.spark, "var(--color-accent, #2563EB)")}
        </div>
        <div class="mt-2 flex items-center gap-1.5">
          <span class="inline-flex items-center gap-0.5 rounded-full ${k.deltaPositive ? "bg-emerald-50 text-emerald-700" : "bg-red-50 text-red-700"} px-1.5 py-0.5 text-[10px] font-semibold tabular-nums">
