-- Inserta permiso con goce (FJ) en dbo.PERMISO (TRESS / datos-analisis) con validaciones.
-- Placeholders tipados (renderizados en Python, no binds ODBC): {{empleado}}, {{usuario}},
-- {{fecha_inicio}}, {{fecha_fin_mostrar}}, {{comentario}}, {{confirmar}}.
...
--
-- Contrato fijo:
--   PM_TIPO   = 'FJ '   (char 3 con espacio)
--   PM_NUMERO = 'PERMISO'
--   PM_CLASIFI = 0      (con goce)
--   PM_COMENTA = comentario tipado (MATRIMONIO / FALLECIMIENTO / PATERNIDAD)
...
DECLARE @Comentario         varchar(30)  = '{{comentario}}';
...
DECLARE @StatusResultado    smallint;
DECLARE @Incidencia         char(3);
...
BEGIN TRY
  ...
  SET @Data =
    'Tipo: FJ - Permiso con goce' + CHAR(13) + CHAR(10) +
    'Motivo: ' + @Comentario + CHAR(13) + CHAR(10) +
  ...
  EXEC dbo.SP_STATUS_INCIDENCIA
    @FechaInicio,
    @Empleado,
    @StatusResultado OUTPUT,
    @Incidencia OUTPUT;

  SET @Mensaje = N'Permiso con goce registrado en TRESS.';
