"""
Repositorio de escritura de vacaciones en SQL Server datos-analisis / TRESS.

Ejecuta el batch ``sql/datos_analisis_insertar_vacacion.sql`` (transacción propia
con validaciones, INSERT VACACION, UPDATE COLABORA y bitácora).
"""
...
_SQL_FILE = Path(__file__).resolve().parent / "sql" / "datos_analisis_insertar_vacacion.sql"
_USUARIO_RE = re.compile(r"^[A-Za-z0-9_]{1,10}$")
logger = logging.getLogger(__name__)


def load_insertar_vacacion_sql() -> str:
    return _SQL_FILE.read_text(encoding="utf-8")


@dataclass(frozen=True, slots=True)
class InsertarVacacionResult:
    ok: bool
...
def _render_insertar_vacacion_sql(
    *,
    empleado: int,
    usuario: str,
    fecha_inicio: date,
    ...
    nom_tipo: int,
    dias_gozo: Decimal | float | int,
    dias_pago: Decimal | float | int,
    confirmar: bool,
) -> str:
    ...
    if not isinstance(nom_tipo, int):
        raise ValueError("nom_tipo invalido")
    gozo = Decimal(str(dias_gozo))
    pago = Decimal(str(dias_pago))
    if gozo < 0 or pago < 0:
        raise ValueError("dias negativos")

    template = load_insertar_vacacion_sql()
    replacements = {
        "{{empleado}}": str(empleado),
        "{{usuario}}": usuario,
        "{{fecha_inicio}}": fecha_inicio.isoformat(),
        "{{fecha_fin_mostrar}}": fecha_fin_mostrar.isoformat(),
        "{{nom_tipo}}": str(nom_tipo),
        "{{dias_gozo}}": format(gozo, "f"),
        "{{dias_pago}}": format(pago, "f"),
    ...
    if "{{" in sql:
        raise RuntimeError("quedaron placeholders sin sustituir en el SQL de vacaciones")
    return sql


def _row_to_result(row: dict[str, Any]) -> InsertarVacacionResult:
    ok_raw = row.get("ok")
...
class DatosAnalisisVacacionesWriteRepository:
    """Ejecuta el batch de inserción de vacaciones en TRESS."""

    def __init__(self, engine: AsyncEngine) -> None:
        self._engine = engine

    async def insertar_vacacion(
        self,
        *,
        empleado: int,
        usuario: str,
        ...
        # AUTOCOMMIT a nivel engine + cursor crudo: el batch gestiona BEGIN/COMMIT
        # propios y hay que drenar todos los resultsets (SPs) antes de cerrar ODBC.
        async with self._engine.connect() as conn:
            raw = await conn.get_raw_connection()
            # aioodbc: AsyncAdapt → .driver_connection; fallback .dbapi_connection
            ...
            # Evitar que SQLAlchemy intente resetear isolation con results pendientes.
            try:
        ...
        if row is None:
            return InsertarVacacionResult(
                ...
                mensaje="TRESS no devolvio resultado al registrar vacaciones.",
                nueva_llave=None,
            )
