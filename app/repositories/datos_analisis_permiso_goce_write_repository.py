"""
Repositorio de escritura de permiso con goce (FJ) en SQL Server datos-analisis / TRESS.

Ejecuta el batch ``sql/datos_analisis_insertar_permiso_goce.sql``.
"""
...
_SQL_FILE = Path(__file__).resolve().parent / "sql" / "datos_analisis_insertar_permiso_goce.sql"
_USUARIO_RE = re.compile(r"^[0-9]{1,5}$")
_COMENTARIO_RE = re.compile(r"^[\w\s.,;:'/\-#%()+=@!?]*$", re.UNICODE)
logger = logging.getLogger(__name__)


def load_insertar_permiso_goce_sql() -> str:
    return _SQL_FILE.read_text(encoding="utf-8")


@dataclass(frozen=True, slots=True)
class InsertarPermisoGoceResult:
    ok: bool
...
def _render_insertar_permiso_goce_sql(
    *,
    empleado: int,
    usuario: str,
    fecha_inicio: date,
    ...
    template = load_insertar_permiso_goce_sql()
    replacements = {
    ...
    if "{{" in sql:
        raise RuntimeError("quedaron placeholders sin sustituir en el SQL de permiso goce")
    return sql


def _row_to_result(row: dict[str, Any]) -> InsertarPermisoGoceResult:
    ok_raw = row.get("ok")
...
class DatosAnalisisPermisoGoceWriteRepository:
    """Ejecuta el batch de inserción de permiso con goce (FJ) en TRESS."""

    def __init__(self, engine: AsyncEngine) -> None:
        self._engine = engine

    async def insertar_permiso_goce(
        self,
        *,
        empleado: int,
        usuario: str,
        ...
        sql = _render_insertar_permiso_goce_sql(
            empleado=empleado,
            usuario=usuario,
        ...
        if row is None:
            return InsertarPermisoGoceResult(
                ...
                mensaje="TRESS no devolvio resultado al registrar permiso con goce.",
                nueva_llave=None,
            )
