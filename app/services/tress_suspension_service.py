"""
Registro de suspension en TRESS (SQL Server datos-analisis).

Se invoca de forma sincrona al crear un evento tipo suspension en faltas/retardos.
"""
...
from app.repositories.datos_analisis_suspension_write_repository import (
    DatosAnalisisSuspensionWriteRepository,
...
def _map_fallo_tress(result: InsertarSuspensionResult) -> None:
    codigo = (result.codigo_error or "").upper()
    detalle = result.mensaje or "No se pudo registrar la suspensión en TRESS."
...
async def registrar_suspension_en_tress(
    *,
    no_empleado: int,
    fecha_inicio: date,
    fecha_fin: date,
    ...
    """
    Inserta suspensión en dbo.PERMISO.
    ...
    if not motivo:
        raise DomainValidationError(detail="El motivo (observaciones) es obligatorio para suspensión.")
    if len(motivo) > 30:
        raise DomainValidationError(detail="El motivo no puede exceder 30 caracteres.")

    if settings.TRESS_ESCRITURA_BLOQUEADA:
        logger.warning(
            "TRESS_ESCRITURA_BLOQUEADA activo: suspensión omitida en TRESS "
            "(empleado=%s, %s..%s)",
            no_empleado,
            fecha_inicio,
            fecha_fin,
    ...
    confirmar = not bool(settings.TRESS_SUSPENSION_DRY_RUN)
    ...
    try:
        repo = DatosAnalisisSuspensionWriteRepository(engine)
        result = await repo.insertar_suspension(
        ...
        logger.exception(
            "Error al registrar suspension en TRESS (empleado=%s, %s..%s)",
            no_empleado,
            fecha_inicio,
            fecha_fin,
        )
        raise ServiceUnavailableError(
            detail=f"Error al registrar suspensión en TRESS: {type(exc).__name__}: {exc}"
        ) from exc
    finally:
        await engine.dispose()

    if settings.TRESS_SUSPENSION_DRY_RUN:
        if not result.ok:
            _map_fallo_tress(result)
        logger.warning(
            "TRESS_SUSPENSION_DRY_RUN activo: suspension validada sin COMMIT "
            "(empleado=%s, %s..%s)",
            no_empleado,
            fecha_inicio,
            fecha_fin,
    ...
    logger.info(
        "Suspension registrada en TRESS llave=%s empleado=%s %s..%s",
        result.nueva_llave,
        no_empleado,
        fecha_inicio,
        fecha_fin,
