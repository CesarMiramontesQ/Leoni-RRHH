...
async def test_agent_chat_success_with_tool(client: AsyncClient, db, empleado_rh):
    ...
    assert "incidencias" in data["message"]["content"].lower()
...
@pytest.mark.asyncio
async def test_fallback_answer_from_tool_json():
    from app.agents.incidencias.agent import _fallback_answer_from_tool_json

    answer = _fallback_answer_from_tool_json('{"total_incidencias": 7}')
